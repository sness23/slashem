#	NetHack Makefile.
#       Makefile.  Generated from Makefile.top by configure.
...
dofiles:
	...
	(cd src; cp $(GAME)  $(FILE_AREA_UNSHARE))
	#cp $(srcdir)/sys/winnt/defaults.nh $(FILE_AREA_UNSHARE)
	cp util/recover$(EXEEXT) $(FILE_AREA_UNSHARE)
	cp $(srcdir)/dat/license $(srcdir)/doc/Guidebook.txt $(FILE_AREA_DOC)
#	cp win/Qt/KDE/kslashem.kdelnk $(KDEDIR)/share/applnk/Games
	-rm -f $(SHELLDIR)/$(SCRIPT)
	sed -e 's;/usr/games/lib/nethackdir;$(FILE_AREA_UNSHARE);' \
		-e 's;HACKDIR/nethack;HACKDIR/$(GAME);' \
		< $(srcdir)/sys/unix/nethack.sh \
		> $(SHELLDIR)/$(SCRIPT)
# set up their permissions
	...
	-$(CHOWN) $(GAMEUID) $(SHELLDIR)/$(SCRIPT)
	$(CHGRP) $(GAMEGRP) $(SHELLDIR)/$(SCRIPT)
	chmod $(EXEPERM) $(SHELLDIR)/$(SCRIPT)
...
install: $(GAME) recover$(EXEEXT) $(CNF_SHARE_DAT) dungeon spec_levs Guidebook.txt
